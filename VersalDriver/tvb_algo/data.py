...
def _cache_folder():
    folder = os.path.expanduser("./cache")
...
def tvb192_weights_lengths():
    ...
    if not os.path.exists(cache_fname):
        ...
        with zipfile.ZipFile(urllib.request.urlretrieve(
            'https://github.com/the-virtual-brain/tvb-data/'
            'raw/master/tvb_data/connectivity/connectivity_192.zip')[0]) as zf:
            np.savez(cache_fname,
...
def tvb998_weights_lengths():
    ...
    if not os.path.exists(cache_fname):
        ...
        with zipfile.ZipFile(urllib.request.urlretrieve(
            'https://github.com/the-virtual-brain/tvb-data/'
            'raw/master/tvb_data/connectivity/connectivity_998.zip')[0]) as zf:
            np.savez(cache_fname,
                     W=np.loadtxt(zf.open('weights.txt')),
                     D=np.loadtxt(zf.open('tract_lengths.txt')))
    npz = np.load(cache_fname)
